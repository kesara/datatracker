...
import debug  # pyflakes:ignore # noqa: F401
...
class IANASyncTests(TestCase):
    def test_protocol_page_sync(self):
        ...
        rfc_names = iana.parse_protocol_page(
            '<html><a href="/go/rfc1234/">RFC 1234</a></html>'
        )
        self.assertEqual(len(rfc_names), 1)
        self.assertEqual(rfc_names[0], "rfc1234")

        iana.update_rfc_log_from_protocol_page(
            rfc_names, timezone.now() - datetime.timedelta(days=1)
        )
        self.assertEqual(
            DocEvent.objects.filter(doc=rfc, type="rfc_in_iana_registry").count(), 1
        )
    ...
    def test_changes_sync(self):
        ...
        e = draft.latest_event(
            StateDocEvent, type="changed_state", state_type="draft-iana-action"
        )
        self.assertEqual(
            e.desc,
    ...
    def test_changes_sync_errors(self):
        ...
        # error response
        data = json.dumps({"error": "I am in error."})
...
"""
        ...
        subjects = (
            subject_template % dict(draft=draft.name, rev=draft.rev),
            "Subject: Vacuous Subject",
        )
        ...
        for subject in subjects:
            for tag in tags:
                for embedded_name in embedded_names:
                    if embedded_name or "Vacuous" not in subject:
                        rtime = (
                            7 * subjects.index(subject)
                            + 5 * tags.index(tag)
                            + embedded_names.index(embedded_name)
                        )
                        person = Person.objects.get(user__username="iana")
                        fromaddr = person.email().formatted_email()
                        msg = msg_template % dict(
                            person=quopri.encodestring(person.name.encode("utf-8")),
                            fromaddr=fromaddr,
                            draft=draft.name,
                            rev=draft.rev,
                            tag=tag,
                        ...
                        doc_name, review_time, by, comment = iana.parse_review_email(
                            msg.encode("utf-8")
                        )
                        ...
                        self.assertEqual(
                            review_time,
                            datetime.datetime(
                                2012, 5, 10, 12, 0, rtime, tzinfo=datetime.UTC
                            ),
                        )
                        self.assertEqual(by, Person.objects.get(user__username="iana"))
                        self.assertIn(
                            "there are no IANA Actions", comment.replace("\n", "")
                        )

                        events_before = DocEvent.objects.filter(
                            doc=draft, type="iana_review"
                        ).count()
    ...
    def test_ingest_review_email(
        self, mock_parse_review_email, mock_add_review_comment
    ):
        ...
        self.assertEqual(
            "Unable to parse message as IANA review email", str(context.exception)
        )
    ...
    def test_notify_page(self):
        ...
        url = urlreverse(
            "ietf.sync.views.notify", kwargs=dict(org="iana", notification="changes")
        )
...
class RFCSyncTests(TestCase):
    ...
    def test_rfc_index(self):
        ...
        draft_doc.action_holders.add(
            draft_doc.ad
        )  # not normally set, but add to be sure it's cleared
...
<rfc-index xmlns="http://www.rfc-editor.org/rfc-index"
    ...
    <rfc-entry>
        ...
        with mock.patch("ietf.sync.rfceditor.log") as mock_log:
            for rfc_number, _, d, rfc_published in rfceditor.update_docs_from_rfc_index(
                data, errata, today - datetime.timedelta(days=30)
            ):
                changes.append(
                    {"doc_pk": d.pk, "rfc_published": rfc_published}
                )  # we ignore the actual change list
                self.assertEqual(rfc_number, 1234)
                if rfc_published:
        ...
        self.assertTrue(
            not os.path.exists(
                os.path.join(settings.INTERNET_DRAFT_PATH, draft_filename)
            )
        )
        self.assertTrue(
            os.path.exists(
                os.path.join(settings.INTERNET_DRAFT_ARCHIVE_DIR, draft_filename)
            )
        )
        ...
        self.assertEqual(
            rfc_doc.author_persons_or_names(), draft_doc.author_persons_or_names()
        )
        ...
        expected_events = [
            [
                ...
                "",
            ],  # Not looking for exact desc match here - see detailed tests below
            [
                "sync_from_rfc_editor",
                "Imported membership of rfc1234 in std2 via sync to the rfc-index",
            ],
            [
                "std_history_marker",
                "No history of STD2 is currently available in the datatracker before this point",
            ],
            [
                "sync_from_rfc_editor",
                "Imported membership of rfc1234 in fyi1 via sync to the rfc-index",
            ],
            [
                "fyi_history_marker",
                "No history of FYI1 is currently available in the datatracker before this point",
            ],
            [
                "sync_from_rfc_editor",
                "Imported membership of rfc1234 in bcp1 via sync to the rfc-index",
            ],
            [
                "bcp_history_marker",
                "No history of BCP1 is currently available in the datatracker before this point",
            ],
        ...
        for index, [event_type, desc] in enumerate(expected_events):
            ...
            if index == 0:
                self.assertIn(
                    "Received changes through RFC Editor sync (created document RFC 1234,",
                    rfc_events[0].desc,
                )
                self.assertIn(
                    f"created became rfc relationship between {rfc_doc.came_from_draft().name} and RFC 1234",
                    rfc_events[0].desc,
                )
                self.assertIn("set title to 'A Testing RFC'", rfc_events[0].desc)
                self.assertIn(
                    "set abstract to 'This is some interesting text.'",
                    rfc_events[0].desc,
                )
                self.assertIn("set pages to 42", rfc_events[0].desc)
                self.assertIn(
                    "set standardization level to Proposed Standard", rfc_events[0].desc
                )
                self.assertIn(
                    f"added RFC published event at {rfc_events[0].time.astimezone(RPC_TZINFO):%Y-%m-%d}",
                    rfc_events[0].desc,
                )
                self.assertIn(
                    "created updates relation between RFC 1234 and RFC 123",
                    rfc_events[0].desc,
                )
        ...
        for subseries_name in ["bcp1", "fyi1", "std2"]:
            sub = Document.objects.filter(
                type_id=subseries_name[:3], name=subseries_name
            ).first()
        ...
        self.assertTrue(
            RelatedDocument.objects.filter(
                source=rfc_doc, target__name="rfc123", relationship="updates"
            ).exists()
        )
        self.assertTrue(
            RelatedDocument.objects.filter(
                source=draft_doc, target=rfc_doc, relationship="became_rfc"
            ).exists()
        )
        ...
        changed = list(
            rfceditor.update_docs_from_rfc_index(
                data, errata, today - datetime.timedelta(days=30)
            )
        )
...
<source>%(group)s</source>
    ...
    def test_rfc_queue(self):
        draft = WgDraftFactory(
            states=[("draft-iesg", "ann")], ad=Person.objects.get(user__username="ad")
        )
        draft.action_holders.add(
            draft.ad
        )  # add an action holder so we can test that it's removed later

        expected_auth48_url = "http://www.rfc-editor.org/auth48/rfc1234"
        t = self._generate_rfc_queue_xml(
            draft, state="EDIT*R*A(1G)", auth48_url=expected_auth48_url
        )
        ...
        self.assertEqual(
            set(draft.tags.all()),
            set(DocTagName.objects.filter(slug__in=("iana", "ref"))),
        )
        ...
        self.assertEqual(
            events[2].type, "changed_state"
        )  # changed draft-rfceditor state
    ...
    def test_rfceditor_parse_queue_TI_state(self):
        ...
        draft = WgDraftFactory(states=[("draft-iesg", "ann")])
        t = self._generate_rfc_queue_xml(
            draft, state="TI", auth48_url="http://www.rfc-editor.org/auth48/rfc1234"
        )
    ...
    def test_update_draft_auth48_url(self):
        ...
        changed, warnings = rfceditor.update_drafts_from_queue(
            self._generate_rfceditor_update(
                draft, state="AUTH48", auth48_url="http://www.rfc-editor.org/rfc1234"
            )
        )
        self.assertEqual(len(changed), 1)
        self.assertEqual(len(warnings), 0)
    ...
    def test_post_approved_draft_in_production_only(self):
        self.requests_mock.post(
            "https://rfceditor.example.com/", status_code=200, text="OK"
        )

        # be careful playing with SERVER_MODE!
        with override_settings(SERVER_MODE="test"):
            self.assertEqual(
                rfceditor.post_approved_draft(
                    "https://rfceditor.example.com/", "some-draft"
                ),
                ("", ""),
            )
...
class DiscrepanciesTests(TestCase):
    def test_discrepancies(self):
        ...
        doc.set_state(
            State.objects.get(used=True, type="draft-iana-action", slug="inprog")
        )
        ...
        doc.set_state(State.objects.get(used=True, type="draft-iesg", slug="rfcqueue"))
        doc.set_state(
            State.objects.get(used=True, type="draft-iana-action", slug="waitrfc")
        )
...
class ErrataTests(TestCase):
    ...
    def test_errata_map_from_json(self):
        input_data = [
            ...
            {"doc-id": "RFC1001", "errata_status_code": "Verified"},
            {
                "doc-id": "RfC1234",
                "errata_status_code": "Verified",
            },
...
class TaskTests(TestCase):
    ...
    def test_rfc_editor_index_update_task(
        ...
        None
    ):  # the annotation here prevents mypy from complaining about annotation-unchecked
        ...
        parse_index_mock.return_value = MockIndexData(
            length=rfceditor.MIN_INDEX_RESULTS
        )
        ...
        self.assertEqual(
            (([rfc.rfc_number],), {}), (rsync_task_args, rsync_task_kwargs)
        )
        ...
        parse_index_mock.return_value = MockIndexData(
            length=rfceditor.MIN_INDEX_RESULTS - 1
        )
        tasks.rfc_editor_index_update_task(full_index=False)
        self.assertTrue(parse_index_mock.called)
        self.assertFalse(update_docs_mock.called)
    ...
    def test_rfc_editor_queue_updates_task(self, mock_parse, mock_update):
        ...
        self.requests_mock.get(
            "https://rfc-editor.example.com/queue/", exc=requests.exceptions.Timeout
        )
        ...
        self.requests_mock.get(
            "https://rfc-editor.example.com/queue/", text="the response"
        )

        # mock returning < MIN_QUEUE_RESULTS values - treated as an error, so no update takes place
        mock_parse.return_value = (
            [n for n in range(rfceditor.MIN_QUEUE_RESULTS - 1)],
            ["a warning"],
        )
        tasks.rfc_editor_queue_updates_task()
        ...
        mock_parse.return_value = (
            [n for n in range(rfceditor.MIN_QUEUE_RESULTS)],
            ["a warning"],
        )
        ...
        self.assertEqual(
            ...
            mock.call([n for n in range(rfceditor.MIN_QUEUE_RESULTS)]),
        )
    ...
    def test_iana_protocols_update_task(
        ...
        self.assertEqual(
            published_later_than, {datetime.datetime(2012, 11, 26, tzinfo=datetime.UTC)}
        )
